...
/**
 * Stores {@link EventType} in the column using the contract's spelling rather than Java's.
 *
 * <p>{@code @Enumerated(STRING)} would write {@code TRANSACTION_CREATED}. The event envelope schema
 * says {@code transaction.created}, the {@code CHECK} constraint says the same, and the contract wins:
...
public class EventTypeConverter implements AttributeConverter<EventType, String> {
    ...
    public EventType convertToEntityAttribute(String dbData) {
        return dbData == null ? null : EventType.fromWireValue(dbData);
    }
}
