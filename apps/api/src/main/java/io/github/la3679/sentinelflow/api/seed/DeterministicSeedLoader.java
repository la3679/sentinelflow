...
public class DeterministicSeedLoader {
    ...
    public DeterministicSeedLoader(EntityManager entityManager, PasswordEncoder passwordEncoder) {
    ...
    public SeedManifest load(SeedProperties properties) {
        ...
        int analysts = seedAnalysts(references, properties.operatorPassword());
    ...
    /**
     * The four demo operators, each with a role and a credential.
     *
     * <p>The password is the same for all four and comes from configuration, never from this file
     * and never from a migration (ADR-0012 §6). One shared password is right for a demo whose whole
     * user directory is generated by a command: four different generated passwords would be four
     * things to look up in {@code .env} to click through one workflow.
     *
     * <p><strong>The hash is not part of the manifest checksum, deliberately.</strong> BCrypt salts
     * randomly, so no two runs produce the same hash even from the same password — including the hash
     * would make the checksum differ on every run and stop it meaning "the same dataset". The
     * checksum covers the generated references, which is what determinism is a claim about.
     */
    private int seedAnalysts(List<String> references, String operatorPassword) {
        // Encoded once. BCrypt is deliberately slow, and four calls for four
        // identical passwords would be three of them spent proving that.
